...
- `platform-app-service/`: reusable service chart used by application `HelmRelease` resources.
...
The `platform-app-service` chart is intended to be published to ACR as an OCI artifact:

```bash
helm package helm/platform-app-service --destination .artifacts/helm
helm push .artifacts/helm/platform-app-service-0.1.0.tgz oci://<acr-name>.azurecr.io/helm
```
